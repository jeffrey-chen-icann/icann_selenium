...
import com.icann.env.EnvironmentOO;

public class _SmokeLoginOO {
    static WebDriver browser; 
    static EnvironmentOO env;
	
	@BeforeClass
	static public void runOnce() throws Exception {
		browser = Environment.initializeDriver();
		env = EnvironmentOO.setEnvironment();	
	...
	public void loginWithAdmin() {
		...
		Helper.logTestStep("Log in with an admin user:  " + env.sDmsAdminUsername());
		Dms.login(env.sDmsAdminUsername(), env.sDmsAdminPassword());
	...
	public void loginWithNonAdmin() {
		...
		Helper.logTestStep("Log in with a non-admin user:  " + env.sDmsNonAdminUsername());
		Dms.login(env.sDmsNonAdminUsername(), env.sDmsNonAdminPassword());
	...
	public void loginWithInvalidUsernamePassword() {
		...
		if (!browser.getCurrentUrl().contains("login")) { 
			Helper.logTestStep("Open:  " + env.sDmsUrl());
			browser.navigate().to(env.sDmsUrl());
		}
		
		Helper.logTestStep("Fill out login:  " + sBogusUsername + " / " + sBogusPassword);
		Login.fillOutLogin(sBogusUsername, sBogusPassword);
		
    	Helper.logTestStep("Click the Sign In button.");
        Helper.waitForThenClick(Login.btnSignInButton);
		
		Helper.logTestStep("Verify an error message is shown:  " + Login.sUnknownUsernameOrPasswordMessage);
		Helper.compareStrings(Login.sUnknownUsernameOrPasswordMessage, Helper.waitForElement(Login.txtErrorMessage).getText());
		...
		Helper.waitForElement(Login.txtPassword);
	}
